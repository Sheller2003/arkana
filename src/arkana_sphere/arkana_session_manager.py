...
class ArkanaSessionManager:
    ...
    def get_session_files(
        ...
        for session_data in self.list_sessions():
            if str(session_data.get("arkana_object_id")) != str(arkana_object_id):
                continue
            if str(session_data.get("user_id")) != str(user_object.user_id):
                continue

            workspace_path_value = session_data.get("workspace_path")
            if not workspace_path_value:
                continue

            workspace_path = Path(str(workspace_path_value))
                ...
                files.append(
                    {
                        ...
                        "runtime_type": str(session_data.get("runtime_type") or "py"),
                        "file_name": path.name,
